...
def run_ss_2(strategy_details, open_permid = None, close_permid = None):
    if open_permid is not None or close_permid is not None:
        # check trades first
        executed = ib.trades()
        open_trade = next((trade for trade in executed if trade.order.permId == open_permid), None)
        close_trade = next((trade for trade in executed if trade.order.permId == close_permid), None)
    else:
        open_trade = None
        close_trade = None
    ...
    if LIVE:
        push_notifications(
            f"{strategy_details} / {open_permid} / {close_permid}"
        )

        push_notifications(f"OPEN TRADE:: {open_trade}")
        push_notifications(f"CLOSE TRADE:: {close_trade}")
    else:
    ...
    while ib.sleep(1):
        ...
        if open_trade is None and close_trade is None:
            ...
            if LIVE:
                ...
                print(f"OPEN ORDER PLACED :: {open_order}")     
                print("Not live (0)")
        ...
        if open_trade is not None:
            ...
            if (
                ...
                if LIVE:
                    ...
                    print(f"OPEN TRADE CANCELLED:: {open_trade.order}")
                    print('Not live (1)')

                print("Order cancelled/inactive. Resetting open_trade to None")
                open_trade = None
                ...
                if (
                    ...
                    if LIVE:
                        ...
                        ib.sleep(1)
                        push_notifications(f"MODIFIED ORDER PLACED:: {open_trade.order}")
                    else:
                        print(f"MODIFIED ORDER PLACED:: {open_trade.order}")
                        print("Not live (2)")
        ...
        if close_trade is not None:
            if close_trade.orderStatus.status == "Filled":
                ...
                if LIVE:
                    ...
                    print(f"[NOT LIVE] CLOSE TRADE FILLED:: {close_trade.order}")
                ...
                if LIVE:
                    push_notifications(f"CLOSE TRADE CANCELLED:: {close_trade.order}")
                else:
                    print(f"CLOSE TRADE CANCELLED:: {close_trade.order}")
                    print('Not live (11)')
...
def run_simple_scalp(strategy_details, open_permId, close_permId, cancel_permids=[]):
    open_trade = None
    close_trade = None
    open_order = None
    close_order = None
